---
title: 'Datenschutz'
layout: '~/layouts/MarkdownLayout.astro'
---

_Letzte Aktualisierung_: Januar 01, 2025

## 1. Verantwortliche Stelle
Die verantwortliche Stelle für die Datenverarbeitung auf dieser Webseite ist:  

**Stefanie Leidel**  
**Gesundheitspraxis | Stefanie Leidel**  
**Unterdorf 24**  
**64823, Gross-Umstadt, Deutschland**  
**E-Mail:**  
steffi@wohlfühlgesundheit.de



## 2. Erfassung allgemeiner Informationen
Wenn Sie auf unsere Webseite zugreifen, werden automatisch Informationen allgemeiner Natur erfasst. Diese Informationen beinhalten etwa die Art des Webbrowsers, das verwendete Betriebssystem, den Domainnamen Ihres Internet-Service-Providers und Ähnliches. Hierbei handelt es sich ausschließlich um Informationen, welche keine Rückschlüsse auf Ihre Person zulassen.

## 3. Erhebung und Verarbeitung personenbezogener Daten
Personenbezogene Daten werden nur erhoben, wenn Sie uns diese von sich aus mitteilen, z.B. durch das Ausfüllen eines Kontaktformulars oder das Senden von E-Mails. Wir verwenden diese Daten ausschließlich zur Beantwortung Ihrer Anfragen und zur technischen Administration.

## 4. Weitergabe personenbezogener Daten
Eine Weitergabe Ihrer Daten an Dritte findet nicht statt, es sei denn, wir sind gesetzlich dazu verpflichtet.

## 5. Cookies
Unsere Webseite verwendet Cookies. Dies sind kleine Textdateien, die auf Ihrem Endgerät gespeichert werden. Cookies richten keinen Schaden an. Einige Cookies bleiben auf Ihrem Endgerät gespeichert, bis Sie diese löschen. Sie ermöglichen es uns, Ihren Browser beim nächsten Besuch wiederzuerkennen. Wenn Sie dies nicht wünschen, können Sie Ihren Browser so einrichten, dass er Sie über das Setzen von Cookies informiert und Sie dies nur im Einzelfall erlauben.

## 6. Ihre Rechte
Sie haben das Recht, jederzeit Auskunft über Ihre bei uns gespeicherten personenbezogenen Daten zu erhalten. Ebenso haben Sie das Recht auf Berichtigung, Sperrung oder Löschung dieser Daten. Kontaktieren Sie uns dazu bitte unter den oben angegebenen Kontaktdaten.

## 7. Änderungen unserer Datenschutzerklärung
Wir behalten uns vor, diese Datenschutzerklärung gelegentlich anzupassen, damit sie stets den aktuellen rechtlichen Anforderungen entspricht oder um Änderungen unserer Leistungen in der Datenschutzerklärung umzusetzen. Für Ihren erneuten Besuch gilt dann die neue Datenschutzerklärung.
